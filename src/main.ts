...
async function run() {
  const changesRequired = Boolean(JSON.parse(core.getInput("request-changes",  { required: true})));
  ...
  if (!Number.isNaN(prNumber)) {
    await requestChanges(changesRequired, token, github.context, commentBody, prNumber);
  } else {
